            ...
            <div class="mining-data">
              ...
              <table class="data-list" width="100%">
                <tr>
                  ...
                  <td width="33%">每块投入</td>
                  <td width="33%" class="value">{{miningInfo.partialAttempt.toFixed(6)}} ETH</td>
                </tr>
                <tr>
                  <td class="ename"><i class="dot yellow-border"></i>Left Bet</td>
                  <td>剩余投入</td>
                  <td class="value">{{miningInfo.leftEther.toFixed(6)}} ETH</td>
                </tr>
                <tr>
                  <td class="ename"><i class="dot blue-border"></i> Proift</td>
                  <td>总产出</td>
                  <td class="profit-value value">{{miningInfo.profit.toFixed(6)}} BTE</td>
                </tr>
              </table>
            </div>
          </div>
...
export default {
  ...
  data () {
    return {
      ...
      poolContractAddress: ['0x26289A0445F649731DdC510606C90988f5B1A6A2', '0xfB1cAbB706ef3D4b4f5BEec8CF06B301d9E9bdaE'],
      miningInfo: {
        profit: 0,
        leftEther: 0,
        totalEther: 0,
  ...
  methods: {
    getPoolInstances() {
      return [
        window.web3.eth.contract(this.poolContractABI).at(this.poolContractAddress[0]),
        window.web3.eth.contract(this.poolContractABI).at(this.poolContractAddress[1])
      ]
    },
    getDefaultPoolInstances() {
      return [
        window.defaultWeb3.eth.contract(this.poolContractABI).at(this.poolContractAddress[0]),
        window.defaultWeb3.eth.contract(this.poolContractABI).at(this.poolContractAddress[1]),
      ]
    },
    getBteInstance() {
    ...
    loadAccountInfo () {
      ...
      let defaultPoolInstances = this.getDefaultPoolInstances();
      let self = this
      ...
      for (let defaultPoolInstance of defaultPoolInstances) {
        defaultPoolInstance.find_contribution.call(this.currentAccount.address, (err, contribution) => {
          ...
          //ep.actual_attempt
          self.miningInfo.partialAttempt += parseFloat(web3.fromWei(partial_attempt, 'ether'));
          self.miningInfo.leftEther += parseFloat(web3.fromWei(total_contribution_for_epoch_remaining, 'ether'));
    ...
    toBte(value) {
      let v = parseFloat(value.dividedBy(100000000));
      return v;
    },
    searchAccount() {
      this.loadAccountInfo();
